window.onload = function() {
	 ...
	 * 회원탈퇴
	 */ 
	$(document).on("click", ".button-member-exit", function(){
		console.log("회원탈퇴");
		$(".member-derag").show();
	});
	
	$(document).on("click",  ".button-userderag-cancel", function(){
		var isOk = confirm("취소하시겠습니까?");
		
		if(isOk){
			$(this).siblings(".derag-pwd").val("");
			$(".member-derag").hide();
		}
	});
	
	$(document).on("click",  ".button-user-derag", function(){
		var isOk = confirm("정말 탈퇴하시겠습니까?");
		
		if(isOk){
			var inputPwd = $(this).siblings(".derag-pwd").val();
			
			$.ajax({
				url: "/StyleFollow/MemberDeragServlet?inputPwd="+inputPwd,
				type: "post",
				dataType: "json",
				success:function(data){
					if(data.result == 'fail'){
